...
export const server = function (serv, options) {
  ...
  serv.on('seed', (seed) => {
    ...
    const seedHash: bigint = crypto.createHash('sha256').update(seedBuf).digest().subarray(0, 8).readBigInt64LE()
    const bigInt1: bigint = BigInt.asIntN(64, seedHash) < 0 ? -(BigInt.asUintN(32, (-seedHash) >> 32n) + 1n) : seedHash >> 32n
    const bigInt2: bigint = BigInt.asIntN(32, seedHash & (2n ** 32n - 1n))
    serv.hashedSeed = [Number(bigInt1), Number(bigInt2)] // convert BigInt to mcpc long
  })
}

export const player = async function (player, serv, settings) {
