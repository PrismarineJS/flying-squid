import * as prisWorld from 'prismarine-world'
...
// @ts-ignore
export const server = async function (serv, options: Options = { name: 'diamond_square', options: { worldHeight: 80 } }) {
  const {version,worldFolder,generation} = options;
  const World = prisWorld(version)
...
export const player = function (player, serv, settings) {
  ...
  player.sendNearbyChunks = (view, group) => {
    ...
    return spiral([view * 2, view * 2])
      ...
      // @ts-ignore too complex for me to solve rn
