...
module.exports.server = async function (serv, { worldFolder, generation = { name: 'diamond_square', options: { worldHeight: 80 } } } = {}) {
  const newSeed = generation.options.seed || Math.floor(Math.random() * (2 ** 31));
  let seed;
  let regionFolder;
  if (worldFolder) {
...
module.exports.player = function (player, serv, settings) {
  ...
  player.sendChunk = (chunkX, chunkZ, column) => player.behavior('sendChunk', {
    x: chunkX,
    z: chunkZ,
    chunk: column,
  }, ({ x, z, chunk }) => {
  ...
  player.sendMap = () => player.sendNearbyChunks(Math.min(3, settings['view-distance']))
