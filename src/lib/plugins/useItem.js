...
module.exports.player = function (player, serv, { version }) {
  ...
  player._client.on('block_place', ({ direction, location } = {}) => {
    ...
    if (item.name === 'flint_and_steel') { player.use_flint_and_steel(referencePosition, directionVector, position) } else if (item.name === 'spawn_egg') { serv.spawnMob(getEntID(item.nbt.value.EntityTag.value.id.value), player.world, position) }
  })
}
