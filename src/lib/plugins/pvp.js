...
module.exports.player = function (player, serv) {
  ...
  player._client.on('use_entity', ({ mouse, target } = {}) => {
    if (!serv.entities[target]) {
      let dragon;
      for (dragon = target - 1; dragon >= target - 7 && !serv.entities[dragon]; dragon--) break;
...
module.exports.entity = function (entity, serv) {
  entity.takeDamage = ({
    sound = 'game.player.hurt', damage = 1, velocity = new Vec3(0, 0, 0), maxVelocity = new Vec3(4, 4, 4), animation = true,
  }) => {
