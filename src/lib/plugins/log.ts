const fs = require('fs')
const timeStarted = Math.floor((new Date().getTime()) / 1000).toString()
...
export const server = function (serv, settings) {
  ...
  // Causes unnecesary amounts of errors and tbh i dont understand why it's here
  ...
  //     try {
  //       tmp = process.stdout
  //       // @ts-ignore idfk why its erroring here
