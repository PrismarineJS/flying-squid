...
module.exports.server = function (serv) {
  serv.emitParticle = (particle, world, position, {
    whitelist, blacklist = [], radius = 32 * 32, longDistance = true, size = new Vec3(1, 1, 1), count = 1,
  } = {}) => {
...
module.exports.player = function (player, serv) {
  player.commands.add({
    ...
    action({ particle, amount, size }) {
      ...
      serv.emitParticle(particle, player.world, player.position.scaled(1 / 32), { count: amount, size });
    },
  });
};
