...
function tick(redstoneUpdates) {
  redstoneUpdates = redstoneUpdates;
  lateUpdates = [];
  return updateRedstone();
}
...
async function updateRedstone(blocksToChange=[]) {
  ...
  if (!redstoneUpdates.length) {
    ...
    var blocks = await async.map(Object.keys(posList), (pos) => {
      return posList[pos];
    });
    return {
      blocks: blocks,
  ...
  return async.map(redstoneUpdates, async (action) => {
    ...
    if (id == 55) { // Redstone Dust
      return await tickRedstoneDust(action.world, action.position, blocksToChange);
    }
  }).then((results) => {
...
var tickRedstoneDust = async (world, pos, extras) => {
...
function newAction(obj) {
  if (obj.time == 0) redstoneUpdates.push(obj);
  else lateUpdates.push(obj);
}
