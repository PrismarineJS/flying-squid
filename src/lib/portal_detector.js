...
module.exports = {
  detectFrame, findPotentialLines, findBorder, getAir, generateLine, generatePortal, addPortalToWorld, makeWorldWithPortal,
};
...
async function findBorder(world, { line, direction }, directionV) {
  ...
  let right = await findLineInDirection(
    world, bottom[line.length - 1].plus(direction).plus(directionV), 'obsidian',
    directionV, direction.scaled(-1),
  );
  if (left.length == 0 || left.length != right.length) { return null; }
  let top = await findLineInDirection(
    world, left[left.length - 1].plus(direction).plus(directionV), 'obsidian',
...
async function detectFrame(world, startingPoint, directionV) {
  ...
  return asyncFilter(
    (await Promise.all(potentialLines
...
function generatePortal(bottomLeft, direction, width, height) {
  ...
  return {
    ...
    air: flatMap(
      generateLine(bottomLeft.plus(direction).plus(directionV), direction, width - 2),
